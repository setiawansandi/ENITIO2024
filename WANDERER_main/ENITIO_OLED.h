...
public:
  ...
  void display_achievementPage_new(int numDeath, int treasure, int scoreInvicta,
                                   int scoreDynari, int scoreEphilia, int scoreAkrona, int scoreSolaris, String noti, int pageNav)
  {
    display.clearDisplay();
    display.setTextSize(1); // Draw SIZE
    if (noti.length() == 0)
