...
private:
  ...
  unsigned long nextTreasureBuzzTime = 0;
  bool firstRun = true;
  unsigned long endDelay = 0;
  const unsigned long TREASURE_BUZZ_DURATION = 5000; // 5 seconds for treasure buzz
...
public:
 ...
 void handleBuzzer() {
  ...
  // If numL1Treasure is 0, stop any ongoing buzz and reset firstRun
  if (numL1Treasure == 0) {
    if (buzzerEndTime != 0) {
      Player_Buzzer.end_sound();
      buzzerEndTime = 0;
  ...
  if (firstRun) {
    if (currentMillis - previousMillis >= startDelay) {
      ...
      Serial.println("First run delay completed. Next buzz in " + String(endDelay) + "ms");
    }
    return;
  }

   // Handle treasure buzz when numL1Treasure > 0 and first run is complete
  if (currentMillis - previousMillis >= endDelay && buzzerEndTime == 0) {
    // Start a new treasure buzz
    Player_Buzzer.sound(NOTE_E4);
    buzzerEndTime = currentMillis + TREASURE_BUZZ_DURATION;
    previousMillis = currentMillis;
    endDelay = random(3000, 20000);
    Serial.println("Treasure Buzzer started. Next in " + String(endDelay) + "ms");
